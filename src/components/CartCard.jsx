...
const CartCard = ({ item }) => {
  ...
  const remove = (itemIdx) => {
    ...
    toast.error("Supprimé du panier");
  };
  ...
  return (
    <div className="h-80 w-[310px] md:h-72 md:w-[600px] bg-slate-100 dark:bg-[#1f1b24] rounded-2xl hover:shadow-lg mt-[40px] md:mt-[20px]">
      <div className="">
        <div>
          <img src={item.original_picture_url} alt="" width={150} height={150} />
        </div>
          ...
          <div className="flex flex-col ml-[20px] gap-y-2 overflow-y-hidden">
            ...
            <div className="flex justify-evenly gap-x-8 mt-2 md:gap-x-0 md:justify-evenly">
              <div className="mt-[10px] font-bold">
                {item.retail_price_cents}€ {/* Assurez-vous que le prix est correctement converti */}
              </div>
              <div>
                <div className="flex gap-x-6">
                  <p className="flex ">
                    <button className="p-1 mr-2 bg-[#dadada] dark:bg-[#2a2a2a] dark:hover:bg-black dark:border-none border rounded-lg font-bold w-[30px]" onClick={() => decrease(item.id)}>
                      -
                    </button>
                    <span className="text-lg font-bold">{item.qty}</span>
                    <button className="p-1 ml-2 bg-[#dadada] dark:bg-[#2a2a2a] dark:hover:bg-black dark:border-none border rounded-lg font-bold w-[30px]" onClick={() => increase(item.id)}>
                      +
                    </button>
                  </p>
